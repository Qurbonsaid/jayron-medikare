...
import { MedicationCreated, MedicationGetAllReq, MedicationGetAllRes, MedicationGetByIdRes, response } from './types'
...
export const MedicationApi = baseApi.injectEndpoints({
	endpoints: builder => ({
		...
		updateMedication : builder.mutation<response, { id: string; body: Partial<MedicationCreated> }>({
			query: ({ id, body }) => ({
				url: PATH.UPDATE + id,
				method: 'PUT',
				body: body,
