...
export const prescriptionApi = baseApi.injectEndpoints({
	endpoints: builder => ({
		...
		getAllPrescriptions: builder.query<GetAllPresc, Pagination>({
			query:() => ({
				url: PATHS.GET_ALL + `?page=${Pagination.page}&limit=${Pagination.limit}&patient_id=${Pagination.patient_id}&doctor_id=${Pagination.doctor_id}&examination_status=${Pagination.examination_status}&is_roomed=${Pagination.is_roomed}`,
				method: 'GET',
			}),
