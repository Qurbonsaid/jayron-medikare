...
import { Bed, Building2, Calendar, Search, Users } from 'lucide-react';
import { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { PatientBookingsModal, PatientSearchModal } from './components';

const RoomsList = () => {
  const navigate = useNavigate();
  const { canRead } = useRouteActions('/inpatient-calendar');
  ...
  return (
    <div className='container mx-auto p-4 sm:p-6 space-y-4 sm:space-y-6'>
      ...
      <div className='flex flex-col gap-4'>
        ...
        {/* Corpus Tabs */}
        <Tabs value={selectedCorpusId} onValueChange={setSelectedCorpusId}>
        ...
        <Card className='p-8 sm:p-12 text-center'>
          <div className='flex flex-col items-center gap-4 text-red-500'>
            ...
            <p className='text-base sm:text-lg'>
              Хоналарни юклашда хатолик юз берди
            </p>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4'>
          {availableRooms.data.map((room) => {
            ...
            const nextAvailable = room.availability
              ...
              .sort(
                (a, b) =>
                  new Date(b.leave_date!).getTime() -
                  new Date(a.leave_date!).getTime()
              )[0];
                        ...
                        >
                          ...
                          {bed.status === 'available' ? (
                            ...
                            <div className='text-[10px] sm:text-xs'>
                              {bed.leave_date
                                ? format(parseISO(bed.leave_date), 'd MMM', {
                                    locale: uz,
                                  })
                                : 'Банд'}
                            </div>
                  ...
                  {nextAvailable && availableBeds === 0 && (
                    <div className='flex items-center gap-2 text-muted-foreground text-xs sm:text-sm pt-2 border-t'>
                      ...
                      <span>
                        Охирги бўшалиш:{' '}
                        {format(
                          parseISO(nextAvailable.leave_date!),
                          'd MMM, yyyy',
                          { locale: uz }
                        )}
                      </span>
