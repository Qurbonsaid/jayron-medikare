...
import { Clock, Edit, HeartPulse, MoreHorizontal, Plus, Trash2, User } from 'lucide-react'
...
const RoomDetail = () => {
	...
	const [selectedDailyCheckupId, setSelectedDailyCheckupId] = useState<string | null>(null)

	const { id: roomId } = useParams()
	const { data: room, isLoading, refetch } = useGetOneRoomQuery(
		{ id: roomId },
		{ skip: !roomId }
	)
	...
	const patientBloodPressureMap = useMemo(() => {
		const map: Record<string, { hasData: boolean; checkupId: string | null; lastReading?: string }> = {}
											...
											>												<div className='absolute top-4 right-4'>
													<DropdownMenu>
														...
														<DropdownMenuContent>
															<DropdownMenuItem
																onClick={() => {
																	const bpData = patientBloodPressureMap[patientData?._id || '']
															...
															>
																...
																<span>
																	{patientBloodPressureMap[patientData?._id || '']?.hasData
																		? 'o\'zgartirish'
																		: 'qon bosimi'}
																</span>
															</DropdownMenuItem>
