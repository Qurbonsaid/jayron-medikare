...
const Rooms = () => {
  ...
  const { canRead, canCreate } = useRouteActions('/inpatient');
  const { canUpdate, canDelete } = useRouteActions('/inpatient/:id');
  ...
  const { data: getCorpus } = useGetOneCorpusQuery({ id }, { skip: !id });
